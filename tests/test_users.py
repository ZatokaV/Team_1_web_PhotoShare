...
class TestUserCRUD(unittest.TestCase):
    ...
    async def test_get_user_profile_found(self):
        self.user_mock.username = "test_user"
        self.session_mock.query(User).filter(
        ).first.return_value = self.user_mock
        self.session_mock.query(Post).filter(
        ).first.return_value = self.post_mock
    ...
    async def test_get_user_profile_not_found(self):
        self.session_mock.query(User).filter().first.return_value = None
    ...
    async def test_get_all_users(self):
        ...
        self.user_mock.id = 1
        self.session_mock.query().all.return_value = users
