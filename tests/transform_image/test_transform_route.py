...
def post_id(c_user, token, session):
    ...
    if post is None:
        post = Post(photo_url='media/test.jpg', description='My new photo', user_id=cur_user.id)
        session.add(post)
        session.commit()
...
def test_save_transform_image(post_id, client, token):
    url = 'https://res.cloudinary.com/drilpksk7/image/upload/e_grayscale:100/v1/media/test.jpg'
    response = client.post(f'/api/image/transform/save/{post_id}', json={'url': url},
                           headers={"Authorization": f"Bearer {token}"})
...
def test_get_list_of_transformed_for_user(client, token):
    response = client.get(f'/api/image/transform/user', headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200, response.text
    data = response.json()
    assert type(data) == list
